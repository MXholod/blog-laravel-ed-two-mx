...
<div class="tw-container tw-bg-white tw-p-6 tw-rounded-l-lg">
	<h3 class="tw-text-2xl">List of tags</h3>
	
	<div class="tw-mt-2 tw-mb-3 tw-p-2 tw-pl-4" style="background-color: rgb(240 249 255)">
		<p>Here you can add a new tag.
			<a href="{{ route('admin.tags.create') }}" class="tw-font-black tw-text-indigo-600">Add a tag</a>
		</p>
	</div>
		...
		@foreach($tags as $tag)
			@if ($loop->first)
				<li class="list-group-item tw-font-bold">
					Amount of tags in the list: {{ $tags->count() }}
				</li>
			@endif
			<li class="list-group-item">
				...
				<div>
					<div>
						<a href="{{ route('admin.tags.edit',$tag->id) }}" class="tw-mr-4">
							<i class="bi bi-wrench"></i>
						</a>
						<form action="{{ route('admin.tags.destroy', ['tag' => $tag->id]) }}" method="POST" class="tw-inline-block">
								@csrf
								@method('DELETE')
